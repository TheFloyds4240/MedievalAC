...
import static com.github.retrooper.packetevents.protocol.player.ClientVersion.V_1_12_2;
import static java.lang.Math.PI;

public class ReachUtils {
    // Copied from 1.8... I couldn't figure out 1.14+. "Enterprise" java code is unreadable!
    public static Pair<Vector3D, BlockFace> calculateIntercept(SimpleCollisionBox self, Vector3D origin, Vector3D end) {
     ...
     */
    public static Vector3D getIntermediateWithXValue(Vector3D self, Vector3D other, double x) {
        ...
        if (d0 * d0 < 1.0000000116860974E-7D) {
            ...
            return d3 >= 0.0D && d3 <= 1.0D ? newVector3D(self.getX() + d0 * d3, self.getY() + d1 * d3, self.getZ() + d2 * d3) : null;
        }
    }
     ...
     * passed in vector, or null if not possible.
     */
    public static Vector3D getIntermediateWithYValue(Vector3D self, Vector3D other, double y) {
        double d0 = other.getX() - self.getX();
        double d1 = other.getY() - self.getY();
        double d2 = other.getZ() - self.getZ();
     ...
     * passed in vector, or null if not possible.
     */
    public static Vector3D getIntermediateWithZValue(Vector3D self, Vector3D other, double z) {
        double d0 = other.getX() - self.getX();
        double d1 = other.getY() - self.getY();
        double d2 = other.getZ() - self.getZ();
     ...
     */
    private static boolean isVecInYZ(SimpleCollisionBox self, Vector3D vec) {
     ...
     */
    private static boolean isVecInXZ(SimpleCollisionBox self, Vector3D vec) {
     ...
     */
    private static boolean isVecInXY(SimpleCollisionBox self, Vector3D vec) {
    ...
    public static Vector3D getLook(GrimPlayer player, float yaw, float pitch) {
        if (player.getClientVersion().isOlderThanOrEquals(V_1_12_2)) {
            float f = player.trigHandler.cos(-yaw * 0.017453292F - (float) PI);
            float f1 = player.trigHandler.sin(-yaw * 0.017453292F - (float) PI);
            float f2 = -player.trigHandler.cos(-pitch * 0.017453292F);
            float f3 = player.trigHandler.sin(-pitch * 0.017453292F);
            return newVector3D(f1 * f2, f3, f * f2);
        } else {
            ...
            return newVector3D(f3 * f4, -f5, (double) (f2 * f4));
        }
    }

    public static boolean isVecInside(SimpleCollisionBox self, Vector3D vec) {
    ...
    public static double getMinReachToBox(GrimPlayer player, SimpleCollisionBox targetBox) {
        ...
        for (double eyes : possibleEyeHeights) {
            Vector3D from = newVector3D(player.x, player.y + eyes, player.z);
            Vector3D closestPoint = VectorUtils.cutBoxToVector(from, targetBox);
